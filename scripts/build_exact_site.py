...
def process_page(url, relative_target_path):
    ...
    body_attrs_str = body_tag_match.group(1) if body_tag_match else ""
    
    class_match = re.search(r'class=["\']([^"\']+)["\']', body_attrs_str, re.IGNORECASE)
    body_classes = class_match.group(1) if class_match else ""
...
export default function Page() {{
  ...
  return (
    <>
      ...
      <div className="{body_classes}" dangerouslySetInnerHTML={{{{ __html: bodyHTML }}}} />
    </>
  );
}}
"""
    with open(target_filepath, 'w', encoding='utf-8') as f:
        f.write(react_code)
    print(f"Updated Next.js page: {relative_target_path}")
